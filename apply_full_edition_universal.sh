#!/usr/bin/env bash
set -euo pipefail
echo 'Applying FULL EDITION course experience…'
ROOT=$(pwd)
...
insert_imports_routes(){
  local app='src/App.tsx'
  if [ ! -f "$app" ]; then
    echo '⚠ src/App.tsx not found; creating minimal router.'
    ensure_dir "$app"
    cat > "$app" <<'EOF_MINAPP'
...
EOF_MINAPP
    echo '✔ created minimal src/App.tsx'
    return
  fi
  # Add imports if missing
  if ! grep -q "CourseOverview" "$app"; then
    tmp=$(mktemp)
    cat > "$tmp" <<'EOF_IMP'
import CourseOverview from './pages/course/CourseOverview';
import CourseItemPage from './pages/course/CourseItemPage';
...
import CourseDiscussions from './pages/course/CourseDiscussions';
EOF_IMP
    awk -v imp="$(tr '\n' '\\n' < "$tmp")" 'NR==1{print; print imp; next} {print}' "$app" > "$app.new" && mv "$app.new" "$app"
    rm -f "$tmp"
    echo '✔ inserted course imports into src/App.tsx'
  fi
  # Add routes block if missing
  if ! grep -q "/course/:id" "$app"; then
    tmp=$(mktemp)
    cat > "$tmp" <<'EOF_ROUTES'
        <Route path="/course/:id" element={<CourseOverview />} />
        <Route path="/course/:id/lesson/:itemId" element={<CourseItemPage />} />
...
EOF_ROUTES
    awk -v add="$(tr '\n' '\\n' < "$tmp")" '
      /<\/Routes>/ && !done { print add; done=1 }
      { print }
    ' "$app" > "$app.new" && mv "$app.new" "$app"
    rm -f "$tmp"
    echo '✔ inserted course routes into src/App.tsx'
  fi
}

# Write src/store/course.ts
write_file 'src/store/course.ts' <<'EOF_FILE'
...
# Write src/utils/ics.ts
write_file 'src/utils/ics.ts' <<'EOF_FILE'
...
# Write src/layouts/CourseShell.tsx
write_file 'src/layouts/CourseShell.tsx' <<'EOF_FILE'
...
# Write src/components/course/LeftNav.tsx
write_file 'src/components/course/LeftNav.tsx' <<'EOF_FILE'
...
# Write src/components/course/RightSidebar.tsx
write_file 'src/components/course/RightSidebar.tsx' <<'EOF_FILE'
import type { Course } from '../../store/course';
...
# Write src/components/course/CourseItemRenderer.tsx
write_file 'src/components/course/CourseItemRenderer.tsx' <<'EOF_FILE'
...
# Write src/pages/course/CourseOverview.tsx
write_file 'src/pages/course/CourseOverview.tsx' <<'EOF_FILE'
...
# Write src/pages/course/CourseDiscussions.tsx
write_file 'src/pages/course/CourseDiscussions.tsx' <<'EOF_FILE'
...
# Write src/pages/course/CourseItemPage.tsx
write_file 'src/pages/course/CourseItemPage.tsx' <<'EOF_FILE'
...
# Write src/pages/course/CourseGrades.tsx
write_file 'src/pages/course/CourseGrades.tsx' <<'EOF_FILE'
import CourseShell from '../../layouts/CourseShell';
import LeftNav from '../../components/course/LeftNav';
import RightSidebar from '../../components/course/RightSidebar';
...
# Write src/i18n.ts
write_file 'src/i18n.ts' <<'EOF_FILE'
